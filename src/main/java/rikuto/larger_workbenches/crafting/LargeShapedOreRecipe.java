...
public class LargeShapedOreRecipe implements IRecipe {
	...
	public boolean matches(InventoryCrafting matrix, World world) {
		if (!(matrix instanceof InventoryCraftingBase) || (tier != -1 && tier != LargeShapedRecipe.getTierFromSize(matrix.getSizeInventory())))
			return false;
		InventoryCraftingBase crafting = (InventoryCraftingBase)matrix;
		for (int x = 0; x <= crafting.getWidth() - width; x++) {
			for (int y = 0; y <= crafting.getHeight() - height; ++y) {
				if (checkMatch(crafting, x, y, false))
					return true;
				if (mirrored && checkMatch(crafting, x, y, true))
					return true;
			}
		}
		return false;
	...
	private boolean checkMatch(InventoryCraftingBase matrix, int startX, int startY, boolean mirror) {
