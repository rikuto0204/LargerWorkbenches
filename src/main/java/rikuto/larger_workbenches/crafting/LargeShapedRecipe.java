...
public class LargeShapedRecipe implements IRecipe {
	...
	public boolean matches(InventoryCrafting matrix, World world) {
		if (!(matrix instanceof InventoryCraftingBase) || (tier != -1 && tier != getTierFromSize(matrix.getSizeInventory())))
			return false;
		InventoryCraftingBase crafting = (InventoryCraftingBase)matrix;
		for (int i = 0; i <= crafting.getWidth() - recipeWidth; ++i) {
			for (int j = 0; j <= crafting.getHeight() - recipeHeight; ++j) {
				if (checkMatch(crafting, i, j, true)) {
					return true;
				}
				if (checkMatch(crafting, i, j, false)) {
					return true;
				}
			}
		}
	...
	private boolean checkMatch(InventoryCraftingBase matrix, int x, int y, boolean mirrored) {
